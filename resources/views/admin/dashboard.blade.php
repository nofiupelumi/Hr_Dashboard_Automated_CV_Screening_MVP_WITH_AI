{{--
    ...
    Redesigned HR Admin Dashboard — clean, organised, not cluttered
--}}
@extends('layouts.app')
...
<div class="mb-6 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Good {{ now()->hour < 12 ? 'Morning' : (now()->hour < 17 ? 'Afternoon' : 'Evening') }}, {{ auth()->user()->name }} 👋</h1>
        <p class="text-gray-500 text-sm mt-1">{{ now()->format('l, F j, Y') }} &mdash; Here's what's happening today</p>
    </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    <a href="{{ route('admin.appraisals.index') }}"
       class="bg-white rounded-lg shadow p-5 flex items-center gap-4 hover:shadow-md transition {{ $hrStats['overdue_appraisals'] > 0 ? 'border-l-4 border-red-400' : '' }}">
    ...
    <a href="{{ route('admin.compliance.index') }}"
       class="bg-white rounded-lg shadow p-5 flex items-center gap-4 hover:shadow-md transition {{ $hrStats['expiring_compliance'] > 0 ? 'border-l-4 border-orange-400' : '' }}">
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    ...
    <a href="{{ route('admin.applications.index') }}?qualification_status=qualified"
       class="bg-white rounded-lg shadow p-5 flex items-center gap-4 hover:shadow-md transition">
    ...
    <a href="{{ route('admin.applications.index') }}?processing_status=pending"
       class="bg-white rounded-lg shadow p-5 flex items-center gap-4 hover:shadow-md transition">
        <div class="p-3 rounded-lg bg-yellow-100 text-yellow-600 flex-shrink-0">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="p-4">
            @if($upcomingBirthdays->count() > 0)
                <div class="space-y-3">
                    ...
                    @php
                        $birthdayThisYear = \Carbon\Carbon::createFromDate(now()->year, $staff->date_of_birth->month, $staff->date_of_birth->day);
                        if ($birthdayThisYear->isPast() && !$birthdayThisYear->isToday()) $birthdayThisYear->addYear();
                        $daysUntil = now()->startOfDay()->diffInDays($birthdayThisYear->startOfDay());
                        $isToday = $daysUntil === 0;
                    @endphp
                    <a href="{{ route('admin.staff.show', $staff) }}"
                       class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition {{ $isToday ? 'bg-pink-50 border border-pink-200' : '' }}">
                        @if($staff->profile_photo)
                            <img src="{{ Storage::url($staff->profile_photo) }}" class="w-10 h-10 rounded-full object-cover flex-shrink-0">
                        @else
                ...
                @if($upcomingBirthdays->count() > 5)
                    <p class="text-xs text-gray-400 text-center mt-3">+{{ $upcomingBirthdays->count() - 5 }} more upcoming</p>
                @endif
            @else
    ...
    <div class="bg-white rounded-lg shadow">
        ...
        <div class="p-4 space-y-2">
            <a href="{{ route('admin.staff.create') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-blue-100 text-blue-600 rounded"><i class="fas fa-user-plus text-sm"></i></div>
                <span class="text-gray-700">Add New Staff Member</span>
            </a>
            <a href="{{ route('admin.leave.index') }}?status=pending" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-yellow-100 text-yellow-600 rounded"><i class="fas fa-calendar-check text-sm"></i></div>
                <span class="text-gray-700">Review Pending Leave</span>
            </a>
            <a href="{{ route('admin.appraisals.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-purple-100 text-purple-600 rounded"><i class="fas fa-clipboard-list text-sm"></i></div>
                <span class="text-gray-700">View Appraisals</span>
            </a>
            <a href="{{ route('admin.appraisal-schedule.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-indigo-100 text-indigo-600 rounded"><i class="fas fa-calendar-alt text-sm"></i></div>
                <span class="text-gray-700">Appraisal Schedule</span>
            </a>
            <a href="{{ route('admin.employee-rules.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-red-100 text-red-600 rounded"><i class="fas fa-file-pdf text-sm"></i></div>
                <span class="text-gray-700">Employee Rules PDF</span>
            </a>
            <a href="{{ route('admin.pension.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-green-100 text-green-600 rounded"><i class="fas fa-shield-alt text-sm"></i></div>
                <span class="text-gray-700">Pension Records</span>
            </a>
            <a href="{{ route('admin.compliance.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-orange-100 text-orange-600 rounded"><i class="fas fa-shield-alt text-sm"></i></div>
                <span class="text-gray-700">Compliance Documents</span>
            </a>
            <a href="{{ url('/') }}" target="_blank" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition text-sm">
                <div class="p-2 bg-gray-100 text-gray-600 rounded"><i class="fas fa-external-link-alt text-sm"></i></div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-3 text-sm text-gray-600">{{ $app->keywordSet->job_title ?? 'N/A' }}</td>
                    <td class="px-6 py-3">
                    ...
                    <td class="px-6 py-3 text-sm text-gray-500">{{ $app->created_at->diffForHumans() }}</td>
                    <td class="px-6 py-3">
